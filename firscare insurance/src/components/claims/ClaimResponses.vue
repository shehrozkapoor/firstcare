<template>
  ...
  <div class="grid">
    <div class="header col-12">
      <Panel header="Search">
        <SearchForm
          :p_document_id="searchProps.document_id"
          :p_first_name="searchProps.first_name"
          :p_second_name="searchProps.second_name"
          :p_file_id="searchProps.file_id"
          :p_number="searchProps.number"
          :p_start_date="searchProps.start_date"
          :p_end_date="searchProps.end_date"
          :p_selectedInsuracePlan="searchProps.selectedInsuracePlan"
          :p_selectedGender="searchProps.selectedGender"
          @updateProps="updateSearchProps($event)"
        />
        <Divider />
        <div class="flex flex-row justify-content-between">
          <div>
            <Button
              label="  Search "
              class="mr-2 mt-3 mb-3 pi pi-search p-button-primary"
              @click="searchTable"
            />
            <Button
              label=" Reset "
              class="ml-2 mt-3 mb-3 pi pi-times p-button-danger"
              @click="clearProps"
            />
          </div>
        </div>
    ...
    <div class="flex flex-row col-12">
      ...
      <CustomAddBeneficiaryDialog
        :displayMaximizable="display_new_beneficiary"
      ...
      <FileUpload
        mode="basic"
        name="demo[]"
        chooseLabel="Add Bulk Claims"
        url="http://localhost:8000/upload"
        class="mr-2 mt-3 mb-3 p-button-success"
      />
      <Button
        label="Download Sample"
        class="mr-2 mt-3 mb-3 p-button-secondary p-button-outlined"
      />
    </div>
    <div class="col-12">
      <CustomDataTable
        ...
        :sortable="true"
        show_header="false"
        table_related_type="beneficiary"
        :isLoading="isLoading"
        :rowEditor="true"
      />
    </div>
...
<script>
import SearchForm from "@/components/claims/components/SearchForm";
...
export default {
  ...
  async mounted() {
    await this.beneficiaryServices
      .getAllBeneficiary()
      .then((data) => {
