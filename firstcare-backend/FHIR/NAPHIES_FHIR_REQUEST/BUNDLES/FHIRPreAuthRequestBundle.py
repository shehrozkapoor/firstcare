...
class FHIRPreAuthRequestBundle(FHIRRequestBundle,FHIRMessageHeader):
    def __init__(self):
        pass
    
    async def createFhirPreAuthReqBundle(self,claim_obj,claim_type,sub_type):
        ...
        claim = FHIRClaim()
        await claim.sendClaimManual(claim_obj=claim_obj,use="preauthorization",claim_type=claim_type,sub_type=sub_type,processPriority='normal',careTeams=careteams,supportingInfos=supporting_info,diagnosis=diagnosis,items=items)
        ...
        await self.createMessageHeader(id=claim.claim.id,payor_organization=self.payor_organization,request_code="priorauth-request")
